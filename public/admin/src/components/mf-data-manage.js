...
import MfDataTransmit from "/src/components/mf-data-transmit.js";
import MfDataSearch from "/src/components/mf-data-search.js";
import MfDataToolbar from "/src/components/mf-data-toolbar.js";
import MfDataTable from "/src/components/mf-data-table.js";
import MfDataFormDialog from "/src/components/mf-data-form-dialog.js";
    ...
    this.getTableConfig()
      .then((res) => {
        ...
        let propMappings = [
          ...
          "tableColumnOperations:tableOperations",
        ];
        //遍历data,将data中的每个对象赋值到this.data
        for (const [key, value] of Object.entries(data)) {
          //如果有props里的配置，则有限使用props配置
          let lastIndex = propMappings.findIndex((item) =>
            item.endsWith(`:${key}`)
          );
          let propValue = null;
          if (lastIndex > -1) {
  ...
  methods: {
    ...
    getQueryParams() {
      ...
      for (let key in this.query) {
        if (
          this.query[key] !== null &&
          typeof this.query[key] !== "undefined"
        ) {
      ...
      this.$refs.transmit
        ...
        .then((res) => {
          ...
          Object.keys(res.data).forEach((key) => {
            const column = this.tableColumns.filter(
              (item) => key === item.prop || key === item.propAlias
            );
            data.push({
              column: key,
    ...
    initTableOperations() {
      for (
        let index = 0;
        index < this.tableOperations.buttons.length;
        index++
      ) {
        ...
        if (typeof button === "string") {
          ...
          if (
            !isEmpty(attrs) &&
            typeof this.tableOperations.buttons[index] === "object"
          ) {
    ...
    visibleTableColumns(query) {
      ...
      if (!isEmpty(this.tableSelectionExpr)) {
        this.tableSelection = evaluateExpression(
          this.tableSelectionExpr,
          query
        );
      }
      this.tableColumns.forEach((column) => {
        //根据表达式来
        column.visible = column.visibleExpr
          ? evaluateExpression(column.visibleExpr, query)
          : true;
      });
    },
    ...
    handleToolbarClick(name, data) {
      ...
      if (
        (!ids || ids.length == 0) &&
        ["edit", "delete", "status"].indexOf(name) > -1
      ) {
